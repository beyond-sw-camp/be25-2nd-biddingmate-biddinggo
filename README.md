...
#### 윤정윤
> 경매 서비스를 개발하면서 가장 크게 느낀 것은, 단순히 기능을 구현하는 것만으로는 서비스가 완성되지 않는다는 점이었습니다. 처음에는 입찰, 낙찰 같은 기능들을 어떻게 구현할지에만 집중했지만, 조금만 생각해보니 그보다 중요한 것은 유저가 이 서비스를 어떻게 사용할지였습니다.
 구현을 진행하면서 더 크게 막혔던 부분은 기술이 아니라 도메인 자체였습니다. 팀원 모두가 실제 경매 경험이 없었기 때문에, 경매가 어떤 흐름으로 돌아가는지부터 다시 이해해야 했습니다. 낙찰은 어떤 기준으로 결정되는지, 비크리 경매는 무엇인지 같은 기본적인 개념조차 명확하지 않은 상태에서 개발을 진행하려다 보니, 구현을 하면서도 이 로직이 맞는지 확신할 수 없는 상황이 반복되었습니다. 이때 도메인을 제대로 이해하지 못하면 코드가 아무리 맞아도 서비스는 틀릴 수 있다는 것을 체감했고 이후에는 기능 구현보다 먼저 도메인과 서비스 전체 흐름에 대한 이해에 더 많은 시간을 투자해야 하겠다는 생각을 했습니다.
 또 하나 크게 느낀 점은 기술 선택의 기준이었습니다. 이전까지는 성능이 좋거나 유명한 기술을 선택하는 것이 중요하다고 생각했지만, 이번 프로젝트에서는 팀원들이 동일한 환경에서 개발해야 한다는 현실적인 문제가 더 중요했습니. 설정이 복잡하거나 개인 환경에 의존적인 기술은 협업 자체를 어렵게 만들 수 있었습니다. 그래서 Elasticsearch 대신 PostgreSQL을 선택하고, 로컬 설치 부담을 줄이기 위해 Supabase를 사용하는 방향으로 결정하게 되었습니다. 이 경험을 통해 좋은 기술도 중요하지만 팀의 상황에 맞는 기술을 선택하는 것이 더 중요하다는 것을 알게 되었습니다.
